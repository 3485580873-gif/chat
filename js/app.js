document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        // ── 修复 iOS/WebView 切后台再切回时，凡是用了 backdrop-filter 毛玻璃效果的
        //    区域（顶部栏、输入框、信封/问卷/红包等各种弹窗modal……）都会黑屏闪一下的
        //    系统渲染 bug。
        //
        //    之前的方案（切回前台后再重绘一次）只能缩短黑屏时间，没法完全消除，
        //    因为那一下黑屏是系统在还没执行到我们的JS之前、合成毛玻璃图层时自己画出来的。
        //
        //    改用更彻底的做法：切到后台的一瞬间，就把这些毛玻璃元素临时换成
        //    "不透明的纯色背景 + 关闭模糊"，这样即使系统合成图层时出问题，
        //    露出来的也是正确的纯色，而不是黑色；等回到前台并稳定一小段时间后，
        //    再把毛玻璃效果恢复回来 ──
        // ── 真正的黑闪根源很可能在这里 ──
        // index.html 里原来写死了 <meta name="theme-color" content="#000000">。
        // 这个 meta 标签是告诉系统"App 切换动画期间，还没画出网页内容之前，
        // 先用什么颜色垫底"。写死成黑色，就导致每次从后台切回来、
        // 系统还没来得及渲染出网页内容的那一瞬间，垫底色就是纯黑，
        // 顶部栏和输入框那一片刚好在最外层、最先露出来，所以看起来像是它们在"闪黑"。
        // 这跟 backdrop-filter 毛玻璃没关系，是系统过渡动画本身的垫底色问题。
        //
        // 修复：让这个垫底色跟随当前主题（浅色/深色）实时更新，而不是写死黑色。
        (function syncThemeColorMeta() {
            function currentBg() {
                const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
                // 优先取当前生效的 --header-bg / --primary-bg，取不到再退回默认色
                const cs = getComputedStyle(document.documentElement);
                const v = (cs.getPropertyValue('--primary-bg') || '').trim();
                if (v) return v;
            ...
            updateMeta();
            // data-theme（浅色/深色）或 data-color-theme（主题配色）变化时，实时同步
            const mo = new MutationObserver(updateMeta);
            mo.observe(document.documentElement, { attributes: true, attributeFilter: ['data-theme', 'data-color-theme', 'style'] });
            if (window.matchMedia) {
                window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', updateMeta);
            }
            document.addEventListener('visibilitychange', () => {
                if (document.visibilityState === 'visible') updateMeta();
            });
        })();

        (function fixBackdropFilterBlackFlash() {
            const RESTORE_DELAY = 700; // 回到前台后，等待这么久再恢复毛玻璃效果（毫秒）
            let restoreTimer = null;
            const suppressedEls = new Set();

            function themeSolidBg() {
                const isDark = document.documentElement.getAttribute('data-theme') === 'dark';
                return isDark ? '#1e1e1e' : '#ffffff';
            }

            function opaqueVersionOf(el) {
                // 取该元素本来的背景色，把透明度去掉、变成完全不透明，
                // 这样不同元素（header 是白/黑、modal 遮罩是黑等）都能各自还原成合理的纯色，
                // 而不是统一套用一个固定颜色。
            ...
            function suppressBlur(el) {
                if (!el || suppressedEls.has(el)) return;
                suppressedEls.add(el);
                if (el.dataset._bfPrevInline === undefined) {
                    el.dataset._bfPrevInline = el.getAttribute('style') || '';
                }
                const fallback = opaqueVersionOf(el);
                el.style.setProperty('backdrop-filter', 'none', 'important');
                el.style.setProperty('-webkit-backdrop-filter', 'none', 'important');
                el.style.setProperty('background', fallback, 'important');
                el.style.setProperty('background-color', fallback, 'important');
            }

            function restoreBlur(el) {
                if (!el) return;
                const prev = el.dataset._bfPrevInline;
                if (prev !== undefined) {
                    if (prev) el.setAttribute('style', prev);
                    else el.removeAttribute('style');
                    delete el.dataset._bfPrevInline;
                }
                suppressedEls.delete(el);
            }

            function findBlurElements() {
                const found = [];
            ...
            function onHide() {
                try {
                    ...
                    findBlurElements().forEach(suppressBlur);
                } catch (e) { console.warn('[backdropFix] 隐藏时抑制失败:', e); }
            }

            function onShow() {
                try {
                    // 先确保当前可见的毛玻璃元素也都换成纯色（防止刚切回来那一刻就露黑）
                    findBlurElements().forEach(suppressBlur);
                    if (restoreTimer) clearTimeout(restoreTimer);
                    restoreTimer = setTimeout(() => {
                        suppressedEls.forEach(restoreBlur);
                        restoreTimer = null;
                    }, RESTORE_DELAY);
                } catch (e) { console.warn('[backdropFix] 恢复失败:', e); }
            }
