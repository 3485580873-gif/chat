...
(function() {
    ...
    async function doCallRecoverCheck(attempt) {
        ...
        try {
            ...
            // 弹窗询问用户是否恢复通话
            showCallResumeDialog(bestSession, bestKey);
        } catch (e) {
            console.warn('[call-recover] error:', e);
        }
    }
    function showCallResumeDialog(session, sessionKey) {
        function fmt(ms) {
        ...
        const elapsedStr = fmt(Math.max(elapsed, 0));

        // 获取对方头像
        const avImg = document.querySelector('#partner-avatar img,[id*="partner-avatar"] img,.partner-avatar img');
        const avSrc = avImg ? avImg.src : null;
        const partnerName = (typeof settings !== 'undefined' && settings.partnerName) ? settings.partnerName : '对方';

        const overlay = document.createElement('div');
        overlay.id = 'call-resume-overlay-chat';
        overlay.style.cssText = 'position:fixed;inset:0;z-index:99995;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.65);backdrop-filter:blur(22px);-webkit-backdrop-filter:blur(22px);animation:cFi 0.35s ease;';

        overlay.innerHTML = `
            <div style="width:300px;background:linear-gradient(160deg,rgba(255,255,255,0.11),rgba(255,255,255,0.04));border:1px solid rgba(255,255,255,0.18);border-radius:32px;padding:40px 28px 32px;display:flex;flex-direction:column;align-items:center;gap:8px;color:#fff;box-shadow:0 32px 80px rgba(0,0,0,0.55),inset 0 1px 0 rgba(255,255,255,0.15);position:relative;overflow:hidden;">
                <div style="content:'';position:absolute;inset:0;pointer-events:none;background:radial-gradient(ellipse at 50% 0%,rgba(var(--accent-color-rgb,224,105,138),0.28),transparent 65%);"></div>
                <div style="width:72px;height:72px;border-radius:50%;background:rgba(var(--accent-color-rgb,224,105,138),0.18);display:flex;align-items:center;justify-content:center;margin-bottom:6px;position:relative;">
                    <i class="fas fa-phone-alt" style="font-size:30px;color:var(--accent-color,#e0698a);"></i>
                    <div style="position:absolute;inset:-8px;border-radius:50%;border:1.5px solid rgba(255,255,255,0.15);animation:cRp 2.2s ease-in-out infinite;"></div>
                </div>
                <div style="width:64px;height:64px;border-radius:50%;background:var(--accent-color,#e0698a);overflow:hidden;display:flex;align-items:center;justify-content:center;border:2px solid rgba(255,255,255,0.25);box-shadow:0 6px 22px rgba(0,0,0,0.4);">
                    ${avSrc ? `<img src="${avSrc}" style="width:100%;height:100%;object-fit:cover;">` : '<i class="fas fa-user" style="font-size:24px;color:rgba(255,255,255,0.82);"></i>'}
                </div>
                <div style="font-size:20px;font-weight:700;margin-top:6px;">${partnerName}</div>
                <div style="font-size:28px;font-weight:800;letter-spacing:0.04em;color:rgba(255,255,255,0.95);font-variant-numeric:tabular-nums;margin-top:2px;">${elapsedStr}</div>
                <div style="font-size:12.5px;color:rgba(255,255,255,0.48);margin-top:2px;">上次通话因页面退出而中断</div>
                <div style="display:flex;gap:16px;margin-top:24px;width:100%;">
                    <button id="_call_resume_end" style="flex:1;padding:13px 0;border:1px solid rgba(255,255,255,0.15);border-radius:14px;font-size:14px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:7px;transition:transform 0.15s,box-shadow 0.2s;font-family:var(--font-family,inherit);background:rgba(255,255,255,0.1);color:rgba(255,255,255,0.7);">
                        <i class="fas fa-phone-slash" style="font-size:13px;"></i>结束通话
                    </button>
                    <button id="_call_resume_continue" style="flex:1;padding:13px 0;border:none;border-radius:14px;font-size:14px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:7px;transition:transform 0.15s,box-shadow 0.2s;font-family:var(--font-family,inherit);background:linear-gradient(135deg,#4caf50,#2e7d32);color:#fff;box-shadow:0 6px 20px rgba(76,175,80,0.4);">
        ...
        // hover effects
        overlay.querySelector('#_call_resume_end').onmouseover = function() { this.style.background='rgba(255,82,82,0.3)';this.style.color='#fff';this.style.borderColor='rgba(255,82,82,0.4)'; };
        overlay.querySelector('#_call_resume_end').onmouseout = function() { this.style.background='rgba(255,255,255,0.1)';this.style.color='rgba(255,255,255,0.7)';this.style.borderColor='rgba(255,255,255,0.15)'; };
        overlay.querySelector('#_call_resume_continue').onmouseover = function() { this.style.boxShadow='0 8px 26px rgba(76,175,80,0.55)'; };
        overlay.querySelector('#_call_resume_continue').onmouseout = function() { this.style.boxShadow='0 6px 20px rgba(76,175,80,0.4)'; };

        function closeDialog() { overlay.remove(); }

        overlay.querySelector('#_call_resume_continue').onclick = async function() {
            const ok = window._callModule && window._callModule.resumeFromSession(bestSession);
            if (ok) {
                if (typeof showNotification === 'function') showNotification('通话已恢复 · 继续与' + partnerName + '的视频通话', 'success', 3000);
            } else {
                await localforage.removeItem(sessionKey).catch(() => {});
                if (typeof showNotification === 'function') showNotification('通话恢复失败', 'error', 2000);
            }
            closeDialog();
        };

        overlay.querySelector('#_call_resume_end').onclick = async function() {
            // 记录通话结束事件
            const endElapsed = session.startTs ? Date.now() - session.startTs : 0;
            if (endElapsed > 2000 && typeof window._addCallEvent === 'function') {
                window._addCallEvent('fa-video', '视频通话已结束', fmt(endElapsed));
            }
            await localforage.removeItem(sessionKey).catch(() => {});
            if (typeof showNotification === 'function') showNotification('通话已结束 · ' + fmt(endElapsed), 'info', 3000);
            closeDialog();
        };
