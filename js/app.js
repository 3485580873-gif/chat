document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        //    系统渲染 bug：切回前台时，扫描页面上所有正在用毛玻璃效果的元素，
        //    强制重新触发一次图层重绘 ──
        (function fixBackdropFilterBlackFlash() {
            function forceRepaint(el) {
                if (!el) return;
                const prevCss = el.style.backdropFilter;
                const prevWebkitCss = el.style.webkitBackdropFilter;
                el.style.backdropFilter = 'none';
                el.style.webkitBackdropFilter = 'none';
                requestAnimationFrame(() => {
                    requestAnimationFrame(() => {
                        el.style.backdropFilter = prevCss || '';
                        el.style.webkitBackdropFilter = prevWebkitCss || '';
                    });
                });
            }
            function repaintChrome() {
                try {
                    // 扫描当前页面上所有可见、且计算样式里带 backdrop-filter 的元素
                    // （包括 header、输入框、以及信封/问卷/红包/日记等各种 modal 弹窗）
                ...
                } catch (e) { console.warn('[repaintChrome] 重绘失败:', e); }
            }
            document.addEventListener('visibilitychange', () => {
                if (document.visibilityState === 'visible') repaintChrome();
            });
            window.addEventListener('pageshow', repaintChrome);
            window.addEventListener('focus', repaintChrome);
        })();
