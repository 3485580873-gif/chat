    ...
    stickerLibrary.forEach(src => {
        ...
        item.onclick = () => {
            if (isBatchMode) {
                ...
                // 显示预览区域（可单独发或与文字一起发）
                if (typeof window.setChatStickerPreview === 'function') {
...
function showVoiceTab() {
    ...
    voices.forEach((voice) => {
        const item = document.createElement('div');
        item.style.cssText = 'display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--primary-bg);border:1px solid var(--border-color);border-radius:12px;cursor:pointer;transition:all 0.2s;font-family:var(--font-family);';
        item.innerHTML = `
            <div style="width:32px;height:32px;border-radius:50%;background:var(--accent-color);display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#fff;font-size:13px;"><i class="fas fa-play"></i></div>
            <div style="flex:1;min-width:0;"><div style="font-size:13px;font-weight:600;color:var(--text-primary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${escapeHtml(voice.text || '未命名语音')}</div></div>
            <i class="fas fa-paper-plane" style="color:var(--text-secondary);font-size:12px;flex-shrink:0;"></i>
        `;
        item.addEventListener('mouseover', () => { item.style.borderColor = 'var(--accent-color)'; item.style.background = 'rgba(var(--accent-color-rgb,180,140,100),0.06)'; });
        item.addEventListener('mouseout', () => { item.style.borderColor = 'var(--border-color)'; item.style.background = 'var(--primary-bg)'; });
        item.onclick = () => {
            const voiceMsg = { id: Date.now(), sender: 'user', text: '', timestamp: new Date(), type: 'voice', voiceUrl: voice.audioUrl, voiceText: voice.text, voiceDuration: 0, status: 'sent', favorited: false, note: null, replyTo: null };
            let sent = false, sendTimer = null;
            const doSend = () => { if (sent) return; sent = true; if (sendTimer) { clearTimeout(sendTimer); sendTimer = null; } addMessage(voiceMsg); };
            if (voice.audioUrl) {
                try {
                    const tmpAudio = new Audio(voice.audioUrl);
                    tmpAudio.addEventListener('loadedmetadata', () => { voiceMsg.voiceDuration = Math.round(tmpAudio.duration) || 0; doSend(); });
