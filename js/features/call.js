...
#call-resume-overlay{position:fixed;inset:0;z-index:99995;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,.65);backdrop-filter:blur(22px);-webkit-backdrop-filter:blur(22px);}
#call-resume-overlay.visible{display:flex;animation:cFi .35s ease;}
.call-resume-card{width:300px;background:linear-gradient(160deg,rgba(255,255,255,.11),rgba(255,255,255,.04));border:1px solid rgba(255,255,255,.18);border-radius:32px;padding:40px 28px 32px;display:flex;flex-direction:column;align-items:center;gap:8px;color:#fff;box-shadow:0 32px 80px rgba(0,0,0,.55),inset 0 1px 0 rgba(255,255,255,.15);animation:cCu .45s cubic-bezier(.22,1,.36,1);position:relative;overflow:hidden;}
.call-resume-card::before{content:'';position:absolute;inset:0;pointer-events:none;background:radial-gradient(ellipse at 50% 0%,rgba(var(--accent-color-rgb,224,105,138),.28),transparent 65%);}
.call-resume-icon-wrap{width:72px;height:72px;border-radius:50%;background:rgba(var(--accent-color-rgb,224,105,138),.18);display:flex;align-items:center;justify-content:center;margin-bottom:6px;position:relative;}
.call-resume-icon-wrap i{font-size:30px;color:var(--accent-color,#e0698a);}
.call-resume-icon-wrap::after{content:'';position:absolute;inset:-8px;border-radius:50%;border:1.5px solid rgba(255,255,255,.15);animation:cRp 2.2s ease-in-out infinite;}
.call-resume-avatar{width:64px;height:64px;border-radius:50%;background:var(--accent-color,#e0698a);overflow:hidden;display:flex;align-items:center;justify-content:center;border:2px solid rgba(255,255,255,.25);box-shadow:0 6px 22px rgba(0,0,0,.4);}
...
.call-resume-duration{font-size:28px;font-weight:800;letter-spacing:.04em;color:rgba(255,255,255,.95);font-variant-numeric:tabular-nums;margin-top:2px;}
...
.call-resume-btn{flex:1;padding:13px 0;border:none;border-radius:14px;font-size:14px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:7px;transition:transform .15s,box-shadow .2s;font-family:inherit;}
.call-resume-btn:active{transform:scale(.95);}
.call-resume-continue{background:linear-gradient(135deg,#4caf50,#2e7d32);color:#fff;box-shadow:0 6px 20px rgba(76,175,80,.4);}
.call-resume-continue:hover{box-shadow:0 8px 26px rgba(76,175,80,.55);}
.call-resume-end{background:rgba(255,255,255,.1);color:rgba(255,255,255,.7);border:1px solid rgba(255,255,255,.15);}
...
<div id="call-resume-overlay">
  <div class="call-resume-card">
    ...
    function saveSession() {
        ...
        try { localStorage.setItem('callInterruptedSession', JSON.stringify(session)); } catch(_) {}
    }
    function clearSession() {
        try { localStorage.removeItem('callInterruptedSession'); } catch(_) {}
    }
    function checkAndResumeCall() {
        let session = null;
        try { session = JSON.parse(localStorage.getItem('callInterruptedSession')); } catch(_) { return; }
        ...
        if (session.elapsed < 3000) { clearSession(); return; }
    ...
    function bindEvents() {
        ...
        // 挂断按钮：用事件委托保底，防止直接绑定失效
        document.addEventListener('click', function(e) {
            if (e.target.closest('#call-hangup-btn')) { e.stopPropagation(); endCall(); }
            if (e.target.closest('#call-mini-hangup')) { e.stopPropagation(); endCall(); }
        }, true); // capture phase 确保优先处理
    ...
    window.addEventListener('beforeunload', () => { if (S.active) { saveSession(); } });
    window.addEventListener('pagehide', () => { if (S.active && S.startTime) saveSession(); });
    ...
    function init() {
        ...
        const late = () => {
            injectToolbarBtn();
            setTimeout(checkAndResumeCall, 500);
