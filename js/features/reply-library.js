...
function _renderVoiceCard(container, item, idx) {
    ...
    // 点击卡片或左侧圆形按钮切换播放/暂停
    // 跟聊天里的语音气泡用完全一样的 Web Audio API 方案：不触发 iOS 原生媒体会话，
    // 也就不会跟保活音频互相抢占；resume/创建 AudioContext 必须在点击的同一个
    // 事件循环里同步执行，否则 iOS 会认为已经脱离用户手势，导致播放静音且卡住。
    ...
    const ensureVoiceBuffer = async () => {
        if (voiceBuffer || !item.audioUrl) return;
        try {
    ...
    const onVoiceEnded = () => {
        ...
        if (window._currentPlayingVoice === wrapper) window._currentPlayingVoice = null;
        resumeKeepalive();
    };
    ...
    const wrapper = { _stopVoice: stopThisPreview };
    ...
    const togglePlay = (e) => {
        ...
        // 停止其他正在播放的语音（包括聊天里的语音气泡和字卡库里其他预览）
        if (window._currentPlayingVoice && typeof window._currentPlayingVoice._stopVoice === 'function') {
            window._currentPlayingVoice._stopVoice();
        }

        // 关键：AudioContext 的创建/resume 必须在点击的同一个事件循环里同步执行
        ...
        window._currentPlayingVoice = wrapper;
