        ...
        try {
            if (window.localforage) {
                localforage.keys().then(function (keys) {
                    // 逐个串行读取，避免 Promise.all 把所有条目（可能上百MB）同时全部载入内存导致webview崩溃
                    var idx = 0;
                    function next() {
                        if (idx >= keys.length) {
                            applyStats(total, msgs, cfg, media);
                        ...
                        var k = keys[idx++];
                        localforage.getItem(k).then(function (raw) {
                            if (raw != null) {
                                var str = typeof raw === 'string' ? raw : JSON.stringify(raw);
                                var b = (k.length + str.length) * 2;
                                total += b;
                                if (/messages|msgs|session/i.test(k)) msgs += b;
                                else if (/avatar|image|photo|bg|background|wallpaper/i.test(k)) media += b;
                                else cfg += b;
                            }
                            // 显式释放引用，帮助及时GC，降低峰值内存
                            raw = null; str = null;
                            // 每读完一批就先更新一次界面，避免长时间无响应
                            if (idx % 20 === 0) applyStats(total, msgs, cfg, media);
                            next();
                        }).catch(function () { next(); });
                    }
                    next();
