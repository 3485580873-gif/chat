...
if (_chatSettingsEl) _chatSettingsEl.addEventListener('click', () => {
    ...
    setupAvatarFrameSettings();
            ...
            if (uploadFontBtn && localFontInput) {
                uploadFontBtn.addEventListener('click', () => { localFontInput.click(); });
                localFontInput.addEventListener('change', async (e) => {
                    ...
                    if (!validExts.includes(ext)) {
                        showNotification('请选择 TTF/OTF/WOFF/WOFF2 格式的字体文件', 'error'); return;
                    }
                    ...
                    try {
                        ...
                        await localforage.setItem(getStorageKey('localFontFile'), {
                            name: file.name, data: base64, mime: file.type || 'font/' + ext.slice(1)
                        });
                        ...
                        if (localFontName) localFontName.textContent = '✅ 已加载: ' + file.name;
                        throttledSaveData();
            ...
            (async () => {
                try {
                    ...
                    if (saved && saved.data && settings.customFontUrl === '__local__') {
                        ...
                        const blobUrl = URL.createObjectURL(blob);
                        await applyCustomFont(blobUrl);
                        if (localFontName) localFontName.textContent = '✅ 已加载: ' + saved.name;
                    }
                } catch(e) {}
            })();
            ...
            if (followSystemBtn) {
                followSystemBtn.addEventListener('click', async () => {
                    ...
                    if (localFontName) localFontName.textContent = '';
...
const pinyinCardToggle = document.getElementById('pinyin-card-toggle');
if (pinyinCardToggle) {
    pinyinCardToggle.addEventListener('click', () => {
        settings.pinyinCardEnabled = !settings.pinyinCardEnabled;
        const knob = pinyinCardToggle.querySelector('.setting-pill-switch');
        if (knob) knob.classList.toggle('active', settings.pinyinCardEnabled);
        updatePinyinCardUI();
        throttledSaveData();
    });
    if (settings.pinyinCardEnabled) {
        const knob = pinyinCardToggle.querySelector('.setting-pill-switch');
...
if (pinyinCardMinSlider) {
    pinyinCardMinSlider.addEventListener('input', (e) => {
        ...
        if (val > (settings.pinyinCardMax || 3)) { val = settings.pinyinCardMax || 3; pinyinCardMinSlider.value = val; }
...
if (pinyinCardMaxSlider) {
    pinyinCardMaxSlider.addEventListener('input', (e) => {
        let val = parseInt(e.target.value);
        if (val < (settings.pinyinCardMin || 2)) { val = settings.pinyinCardMin || 2; pinyinCardMaxSlider.value = val; }
...
// ── 梦向问卷关闭按钮监听 ──
(function() {
    function bindDQModal() {
        const closeBtn = document.getElementById('close-dq-modal');
        if (closeBtn && !closeBtn._dqBound) {
            closeBtn._dqBound = true;
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => setTimeout(bindDQModal, 500));
    } else {
        setTimeout(bindDQModal, 500);
    }
})();
