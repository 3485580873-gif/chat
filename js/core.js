...
const loadData = async () => {
    try {
        ...
        window.transferData = transferData; // 暴露给red-packet.js跨脚本访问
        // 梦向问卷
        try {
            const savedDQ = await localforage.getItem(getStorageKey('dreamQuestionnaires'));
            if (savedDQ && Array.isArray(savedDQ)) window.dreamQuestionnaires = savedDQ;
            else window.dreamQuestionnaires = window.dreamQuestionnaires || [];
        } catch(e) { window.dreamQuestionnaires = window.dreamQuestionnaires || []; }
        ...
        setTimeout(() => {
            ...
            try {
                if (typeof updatePinyinCardUI === 'function') updatePinyinCardUI();
                // 同步拼字卡开关按钮的 active 状态（setupEventListeners 在 loadData 前运行，初始值为默认 false）
                const pinyinToggleBtn = document.querySelector('#pinyin-card-toggle');
                if (pinyinToggleBtn) pinyinToggleBtn.classList.toggle('active', !!settings.pinyinCardEnabled);
            } catch(e) {}
            ...
            // 恢复本地字体文件（setupEventListeners 运行时 settings 尚未从存储读回，导致判断失败）
            if (settings.customFontUrl === '__local__') {
                (async () => {
                    try {
                        ...
                        if (saved && saved.data) {
                            ...
                            if (typeof applyCustomFont === 'function') await applyCustomFont(blobUrl);
                            const localFontNameEl = document.getElementById('local-font-name');
                            if (localFontNameEl) localFontNameEl.textContent = '✅ 已加载: ' + saved.name;
                        }
                    } catch(e) { console.warn('本地字体恢复失败', e); }
                })();
            }
        }, 100);
...
const saveData = async () => {
    ...
    const promises = [
        ...
        { key: 'transferData',            val: () => localforage.setItem(getStorageKey('transferData'), window.transferData || transferData) },
        { key: 'dreamQuestionnaires',    val: () => localforage.setItem(getStorageKey('dreamQuestionnaires'), window.dreamQuestionnaires || []) },
    ];
...
function createMessageFragment(msg, prevMsg, nextMsg, lastSenderRef) {
    ...
    let content = (!isRedPacket && !isDQCard && msg.text) ? `<div>${msg.text.replace(/\n/g, '<br>')}</div>` : '';
    if (isDQCard) {
    ...
    // 表情包渲染
    if (msg.sticker) {
        content += `<img src="${msg.sticker}" class="message-sticker" alt="表情包" style="max-width:120px;max-height:120px;border-radius:8px;display:block;${msg.text ? 'margin-top:6px;' : ''} cursor:pointer;" onclick="viewImage('${msg.sticker}')">`;
    }
    messageHTML += content;

    const messageDiv = document.createElement('div');
    if (isDQCard || isRedPacket || isImageOnly || isStickerOnly) {
...
const addMessage = (message) => {
    ...
    if (message.sender === 'user' && message.type === 'normal' && typeof window._onUserMessage === 'function') {
        ...
        // 表情包与文字一起发送（点击"+文字发"按钮）
        window.sendStickerWithText = function() {
            const text = DOMElements.messageInput ? DOMElements.messageInput.value.trim() : '';
            const sticker = window._pendingChatSticker;
            if (!sticker) return;
            const messageData = {
                id: Date.now(),
                sender: 'user',
            ...
            window.clearChatStickerPreview();
            currentReplyTo = null;
            updateReplyPreview();
            addMessage(messageData);
            playSound('send');
            window._triggerDelayedReply && window._triggerDelayedReply(true);
        };
