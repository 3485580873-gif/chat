...
const loadData = async () => {
    try {
        ...
        try {
                ...
                (async () => {
                    try {
                        ...
                        if (saved && saved.data) {
                            ...
                            // 同步更新 UI 标签（如果设置面板已打开）
                            const el = document.getElementById('local-font-name');
                            if (el) el.textContent = '✅ 已加载: ' + saved.name;
                        } else {
                            // 没有字体文件数据，清掉失效的标记
            ...
            } else if (settings.customFontUrl) {
    ...
    if (message.sender === 'user' && message.type === 'normal' && typeof window._onUserMessage === 'function') {
        ...
        (function() {
            function wireStickerButtons() {
                const sendOnlyBtn = document.getElementById('sticker-send-only-btn');
                const cancelBtn = document.getElementById('sticker-cancel-btn');
                if (sendOnlyBtn && !sendOnlyBtn._wired) {
                    sendOnlyBtn._wired = true;
                    sendOnlyBtn.addEventListener('click', function() {
                        if (window._pendingChatSticker) {
            ...
            if (document.readyState === 'loading') {
                document.addEventListener('DOMContentLoaded', wireStickerButtons);
            } else {
                setTimeout(wireStickerButtons, 200);
            }
        })();
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            if (settings.pinyinCardEnabled && Math.random() < 0.3) {
                ...
                if (customReplies && customReplies.length > 0) {
                    allCards.push(...customReplies.map(r => String(r || '').trim()).filter(Boolean));
                }
                ...
                if (allCards.length >= 2) {
                    showTypingIndicator();
                    const cardCount = 2 + Math.floor(Math.random() * 2); // 固定2-3句
